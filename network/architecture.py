...
class ResBlock(nn.Module):
    def __init__(self, in_c, out_c, k, s=1, dilation=1, groups=1):
        super().__init__()
        padding = int((k - 1) / 2) * dilation
        self.conv = get_conv2d_layer(in_c=in_c, out_c=out_c, k=k, s=s, 
                        p=padding, dilation=dilation, groups=groups)
